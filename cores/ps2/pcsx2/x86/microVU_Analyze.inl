...
void analyzeReg1(mV, int xReg, microVFreg& vfRead) {
	if (xReg) {
...
void analyzeReg2(mV, int xReg, microVFreg& vfWrite, bool isLowOp)
{
	if (xReg)
	{
...
void analyzeReg3(mV, int xReg, microVFreg& vfRead)
{
	if (xReg)
	{
		if (_bc_x)
...
void analyzeReg4(mV, int xReg, microVFreg& vfRead)
{
	if (xReg)
	{
		mVUstall   = std::max(mVUstall, mVUregs.VF[xReg].w);
...
void analyzeReg5(mV, int xReg, int fxf, microVFreg& vfRead)
{
	if (xReg)
	{
		switch (fxf)
...
void analyzeReg6(mV, int xReg, microVFreg& vfRead)
{
	if (xReg)
	{
...
void analyzeVIreg1(mV, int xReg, microVIreg& viRead)
{
	if (xReg)
	{
...
void analyzeVIreg2(mV, int xReg, microVIreg& viWrite, int aCycles)
{
	if (xReg)
	{
...
void mVUanalyzeFMAC1(mV, int Fd, int Fs, int Ft)
{
	sFLAG.doFlag = 1;
...
void mVUanalyzeFMAC2(mV, int Fs, int Ft)
{
	analyzeReg1(mVU, Fs, mVUup.VF_read[0]);
...
void mVUanalyzeFMAC3(mV, int Fd, int Fs, int Ft)
{
	sFLAG.doFlag = 1;
	analyzeReg1(mVU, Fs, mVUup.VF_read[0]);
...
void mVUanalyzeFMAC4(mV, int Fs, int Ft)
{
	cFLAG.doFlag = 1;
	analyzeReg1(mVU, Fs, mVUup.VF_read[0]);
...
void mVUanalyzeIALU1(mV, int Id, int Is, int It)
{
	if (!Id)
		mVUlow.isNOP = 1;
...
void mVUanalyzeIALU2(mV, int Is, int It)
{
	if (!It)
		mVUlow.isNOP = 1;
	analyzeVIreg1(mVU, Is, mVUlow.VI_read[0]);
...
void mVUanalyzeIADDI(mV, int Is, int It, s16 imm)
{
...
void mVUanalyzeMR32(mV, int Fs, int Ft)
{
	if (!Ft)
	{
		mVUlow.isNOP = 1;
...
void mVUanalyzeFDIV(mV, int Fs, int Fsf, int Ft, int Ftf, u8 xCycles)
{
...
void mVUanalyzeEFU1(mV, int Fs, int Fsf, u8 xCycles)
{
	analyzeReg5(mVU, Fs, Fsf, mVUlow.VF_read[0]);
	analyzePreg(xCycles);
}
...
void mVUanalyzeEFU2(mV, int Fs, u8 xCycles) 
{
...
void mVUanalyzeMOVE(mV, int Fs, int Ft)
{
	if (!Ft || (Ft == Fs))
		mVUlow.isNOP = 1;
	analyzeReg1(mVU, Fs, mVUlow.VF_read[0]);
...
void mVUanalyzeLQ(mV, int Ft, int Is, bool writeIs)
{
	analyzeVIreg1(mVU, Is, mVUlow.VI_read[0]);
	analyzeReg2(mVU, Ft, mVUlow.VF_write, 1);
	if (!Ft)
...
void mVUanalyzeSQ(mV, int Fs, int It, bool writeIt)
{
...
void mVUanalyzeR1(mV, int Fs, int Fsf)
{
	analyzeReg5(mVU, Fs, Fsf, mVUlow.VF_read[0]);
	analyzeRreg();
}
...
void mVUanalyzeR2(mV, int Ft, bool canBeNOP)
{
	if (!Ft)
	{
		if (canBeNOP)
...
void flagSet(mV, bool setMacFlag)
{
	int curPC = iPC;
	int calcOPS = 0;
...
void mVUanalyzeSflag(mV, int It)
{
...
void mVUanalyzeMflag(mV, int Is, int It)
{
	mVUlow.readFlags = true;
	analyzeVIreg1(mVU, Is, mVUlow.VI_read[0]);
	analyzeVIreg2(mVU, It, mVUlow.VI_write, 1);
...
void mVUanalyzeCflag(mV, int It)
{
	mVUinfo.swapOps = 1;
	mVUlow.readFlags = true;
	analyzeVIreg2(mVU, It, mVUlow.VI_write, 1);
...
void mVUanalyzeXGkick(mV, int Fs, int xCycles)
{
	mVUlow.isKick = true;
...
void analyzeBranchVI(mV, int xReg, bool& infoVar)
{
	if (!xReg)
		return;
...
void mVUanalyzeCondBranch1(mV, int Is)
{
	analyzeVIreg1(mVU, Is, mVUlow.VI_read[0]);
...
void mVUanalyzeCondBranch2(mV, int Is, int It)
{
	analyzeVIreg1(mVU, Is, mVUlow.VI_read[0]);
	analyzeVIreg1(mVU, It, mVUlow.VI_read[1]);
	if (!mVUbranchCheck(mVU))
...
void mVUanalyzeNormBranch(mV, int It, bool isBAL)
{
	mVUbranchCheck(mVU);
	if (isBAL)
	{
...
void mVUanalyzeJump(mV, int Is, int It, bool isJALR)
{

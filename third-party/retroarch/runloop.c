...
bool dynamic_verify_hw_context(
...
bool dynamic_request_hw_context(enum retro_hw_context_type type,
      unsigned minor, unsigned major)
{
   switch (type)
   {
...
{
   unsigned p;
   printf("[Environ]:Before &runloop_state\n");
   runloop_state_t *runloop_st            = &runloop_state;
   printf("[Environ]:After &runloop_state\n");
   settings_t         *settings           = config_get_ptr();

   printf("[Environ]:Before runloop_st->system\n");
   rarch_system_info_t *sys_info          = &runloop_st->system;
   printf("[Environ]:After runloop_st->system\n");
   bool ignore_environment_cb             = (runloop_st->flags &
      RUNLOOP_FLAG_IGNORE_ENVIRONMENT_CB) ? true : false;

   printf("[Environ]:Before ignore cb\n");
   if (ignore_environment_cb)
      return false;
   printf("[Environ]:after ignore cb\n");
   ...
   printf("[Environ] runloop_env: return true");
      ...
      bool *load_no_content)
{
   ...
   dylib_t lib;
#endif
   printf("before runloop_state");
   runloop_state_t *runloop_st  = &runloop_state;
   printf("after runloop_state");
